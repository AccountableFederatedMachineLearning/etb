...
public final class AssetTransfer implements ContractInterface {
     ...
     */
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public String AddLog(final Context ctx, final String msg) {
      ChaincodeStub stub = ctx.getStub();
      try {
        ClientIdentity ci = new ClientIdentity(stub);

        stub.putStringState(msg, "true");
        stub.setEvent("AddLogEvent", (ci.getId() + msg).getBytes());
      } catch (Exception e) {
      }
      return msg;
    }
    ...
    public String Claim(final Context ctx, final String claim) {
        ...
        try {
            ClientIdentity ci = new ClientIdentity(stub);
            X509Certificate certificate = ci.getX509Certificate();
            String subjectID = new JcaX509CertificateHolder(certificate).getSubject().toString();
            String issuerID = new JcaX509CertificateHolder(certificate).getIssuer().toString();
            ClaimEvent claimEvent = new ClaimEvent(subjectID, issuerID, claim);

            CompositeKey key = new CompositeKey("claim", subjectID, issuerID, claim);
            stub.putStringState(key.toString(), "It's true!");
            stub.setEvent("NewClaimEvent", genson.serializeBytes(claimEvent));
        } catch (Exception e) {
        }
        return claim;
    }
